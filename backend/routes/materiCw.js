...
const MateriCw = require('../models/materi_cw');
...
router.get('/', async (req, res) => {
  try {
    const modul = await MateriCw.find().sort({ createdAt: -1 });
    res.json(modul);
  } catch (err) {
...
router.delete('/:id', async (req, res) => {
  try {
    const modul = await MateriCw.findByIdAndDelete(req.params.id);
    if (!modul) {

...
const LaporanWR = require("../models/laporan_wr");
...
router.get("/", async (req, res) => {
  try {
    const laporan = await LaporanWR.find().populate("id_siswa", "name").exec();

    console.log("=== HASIL LAPORAN WR YANG DIPOPULATE ===");
...
router.post("/", upload.single("file"), async (req, res) => {
  try {
    ...
    const newLaporan = new LaporanWR({
...
router.delete("/:id", async (req, res) => {
  try {
    await LaporanWR.findByIdAndDelete(req.params.id);
...
router.put("/:id", upload.single("file"), async (req, res) => {
  try {
    ...
    const updated = await LaporanWR.findByIdAndUpdate(
      req.params.id,
      updateData,
      { new: true }
    );

...
const MateriWR = require("../models/materi_wr"); // Ganti model
...
router.get("/", async (req, res) => {
  try {
    const modul = await MateriWR.find().sort({ createdAt: -1 });
    res.json(modul);
  } catch (err) {
    res
      .status(500)
...
router.delete("/:id", async (req, res) => {
  try {
    const modul = await MateriWR.findByIdAndDelete(req.params.id);
    if (!modul) {

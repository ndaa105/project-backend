...
const LaporanCW = require("../models/laporan_cw");
...
router.get("/", async (req, res) => {
  try {
    const laporan = await LaporanCW.find().populate("id_siswa", "name").exec();

    console.log("=== HASIL LAPORAN YANG DIPOPULATE ===");
...
router.post("/", upload.single("file"), async (req, res) => {
  try {
    ...
    const newLaporan = new LaporanCW({
...
router.delete("/:id", async (req, res) => {
  try {
    await LaporanCW.findByIdAndDelete(req.params.id);
...
router.put("/:id", upload.single("file"), async (req, res) => {
  try {
    ...
    const updated = await LaporanCW.findByIdAndUpdate(
      req.params.id,
      updateData,
      { new: true }
    );

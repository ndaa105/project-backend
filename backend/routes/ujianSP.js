...
const { UjianSP, JawabanUjianSP } = require("../models/ujian_sp"); // Ganti model

// Ambil semua soal ujian
router.get("/", async (req, res) => {
  const soal = await UjianSP.find();
  res.json(soal);
});

// Buat soal ujian baru
router.post("/", async (req, res) => {
  const newSoal = new UjianSP(req.body);
  await newSoal.save();
  res.json(newSoal);
});
...
router.get("/jawaban/:id", async (req, res) => {
  const jawaban = await JawabanUjianSP.find({ id_soal: req.params.id });
  res.json(jawaban);
});
...
router.post("/jawaban", upload.single("file"), async (req, res) => {
  try {
    const newJawaban = new JawabanUjianSP({
...
router.delete("/:id", async (req, res) => {
  try {
    await UjianSP.findByIdAndDelete(req.params.id);
    await JawabanUjianSP.deleteMany({ id_soal: req.params.id });
...
router.get("/jawaban_siswa/:nama", async (req, res) => {
  const data = await JawabanUjianSP.find({ nama_siswa: req.params.nama });
  res.json(data);
});
...
router.delete("/jawaban/:id", async (req, res) => {
  try {
    await JawabanUjianSP.findByIdAndDelete(req.params.id);
